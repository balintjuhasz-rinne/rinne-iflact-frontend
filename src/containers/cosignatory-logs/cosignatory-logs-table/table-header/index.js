...
import { setCosignatoryLogsSort } from '../../../../actions/cosignatory.actions';
...
const CompanyLogsThead = () => {
  const { sortConfig } = useSelector(getCosignatoryLogsSelector);
  const dispatch = useDispatch();

  const onSort = (field) => {
    const direction = (sortConfig.field === field) ? sortConfig.direction * -1 : 1;
    dispatch(setCosignatoryLogsSort(field, direction));
  };

  useEffect(() => {
    dispatch(setCosignatoryLogsSort(LOGS_SORT_FIELDS.DATE, -1));
  }, []);
